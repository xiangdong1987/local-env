...
# 安裝memcached mongodb swoole redis
COPY memcached-3.1.3/ /tmp/memcached-3.1.3/
COPY mongodb-1.4.4/ /tmp/mongodb-1.4.4/
COPY swoole-src-4.3.1/ /tmp/swoole-src-4.3.1/
COPY redis-4.0.2/ /tmp/redis-4.0.2/
RUN cd /tmp/memcached-3.1.3/ ;\
    phpize ; \
    ./configure ; \
    make ;\
    make install; \
    cd /tmp/mongodb-1.4.4/ ;\
    phpize ; \
    ./configure ; \
    make ;\
    make install; \
    cd /tmp/swoole-src-4.3.1/ ;\
    phpize ; \
    ./configure ; \
    make ;\
    make install; \
...
RUN mv /usr/local/etc/php/php.ini-production /usr/local/etc/php/php.ini; \
    ...
    echo "env.name = local">>/usr/local/etc/php/php.ini
